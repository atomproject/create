var dragAndDropSetup = function () {
  ...
  function download() {
    ...
    if (subject.tagName.toLowerCase() === 't-form') {
      ...
      var indexFile = getDemoFile(code, subject.name);
    }
    else {
  ...
  function getDemoFile(code, componentName) {
    return '<html><head>' + '<link rel="import" href="' + atomUrl + 'bower_components/t-shared-components/theme.html"><link rel="import" href="../' + componentName + '.html">' + '</head><body><' + componentName + '></' + componentName + '></body></html>';
  }
